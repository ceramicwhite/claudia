...
pub async fn execute_agent(
    ...
    // Create a separate task to handle output streaming
    ...
    // Return the run immediately
    repo.find_run_by_id(run_id)
}

/// Handle agent output in a separate task
async fn handle_agent_output(
    ...
    if let Some(mut child) = child {
        ...
        // Store all output lines for batch database update
        let mut all_output_lines = Vec::new();

        // Process stdout lines
        for line in stdout_lines {
            line_number += 1;
            all_output_lines.push((line_number, line.clone()));

            // Emit event
            app_handle
                .emit(&format!("agent-output-{}", session_id), &line)
                .ok();

            // Persist to file
            persist_agent_output_line(&app_data_dir, run_id, line_number, &line)
                .await
                .ok();
        ...
        // Update database with all output and final status
        {
            let repo = SqliteAgentRepository::new(pool.as_ref().clone());
            
            // Store all output lines
            for (line_num, content) in all_output_lines {
                repo.store_jsonl_output(run_id, line_num, &content).ok();
            }
            
            // Update final status
            if has_usage_limit_error {
                if let Some(reset) = reset_time {
                    repo.update_run_usage_limit(run_id, &reset, false).ok();
                }
            } else {
                repo.update_run_completion(run_id, final_status).ok();
            }
        }
        ...
        app_handle
            .emit(
                &format!("agent-complete-{}", session_id),
                final_status,
            )
            .ok();
    }
